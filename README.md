...
- По умолчанию `camera-worker` транскодирует RTSP-камеры в браузерный H.264 1080p. Это стабильнее для HLS. Если серверу тяжело, можно поставить `CAMERA_RESTREAM_TRANSCODE=0`, но тогда поток камеры должен сам иметь корректные timestamps и совместимый H.264-профиль.
